...
export function FloatingInput({
  ...
  const actualStartElement =
    startElement || (icon && iconPosition === "start" ? icon : null);
  const actualEndElement =
    endElement || (icon && iconPosition === "end" ? icon : null);
  ...
  return (
    <div className={`relative w-full ${containerClassName}`}>
      ...
      >
        ...
        <div
          className={`relative flex-1 flex flex-col ${isTextArea ? "justify-start" : "justify-center"}`}
        >
          <InputComponent
            {...(props as any)}
            type={type}

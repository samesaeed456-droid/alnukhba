...
export default function Layout({ children }: LayoutProps) {
  ...
  const { isCartOpen, setIsCartOpen, isWishlistOpen, setIsWishlistOpen } =
    useStoreUI();
  ...
  const isHeaderMobileHidden = hideHeaderMobilePaths.includes(
    location.pathname,
  );
  ...
  return (
    ...
    >
      ...
      <main
        className={`flex-grow w-full ${!isFooterVisible && isBottomNavVisible ? "pb-20 md:pb-0" : ""}`}
      >
        {children}
      </main>

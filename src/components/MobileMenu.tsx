...
export default React.memo(function MobileMenu({
  isOpen,
  onClose,
}: MobileMenuProps) {
    ...
    return (
      <div key={item.label}>
        ...
        >
          ...
          <ChevronLeft
            className={`w-4 h-4 transition-colors ${isActive ? "text-solar" : "text-slate-300 group-hover:text-solar"}`}
            strokeWidth={1.5}
          />
        </Link>
      </div>
          ...
          >
            ...
            <div className="bg-gradient-to-br from-slate-900 to-black p-3 pt-4 pb-3 text-white relative overflow-hidden border-b border-slate-800">
              ...
              <div className="flex items-center justify-between relative z-10 mb-3">
                ...
                <button
                  onClick={onClose}
                  className="p-1.5 bg-white/10 hover:bg-white/20 rounded-full transition-colors text-white relative flex items-center justify-center backdrop-blur-sm"
                >
              ...
              {user ? (
                <div className="flex flex-col gap-2.5 relative z-10">
                  ...
                  >
                    ...
                    <div>
                      <p className="text-[10px] text-slate-400 mb-0.5 leading-none">
                        مرحباً بك
                      </p>
                      <p className="font-bold text-sm leading-tight text-white group-hover:text-solar transition-colors">
                        {user.name}
                      </p>
                    </div>
                    <ChevronLeft
                      className="w-4 h-4 text-slate-500 mr-auto group-hover:text-solar transition-colors"
                      strokeWidth={1.5}
                    />
                  </Link>
                  ...
                  <div className="bg-gradient-to-r from-slate-800 to-slate-900 border border-slate-700/50 rounded-xl p-2.5 flex items-center justify-between shadow-lg relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-2.5 relative z-10">
                      <div className="w-7 h-7 bg-solar/20 rounded-lg flex items-center justify-center border border-solar/30">
                        <Wallet
                          className="w-3.5 h-3.5 text-solar"
                          strokeWidth={1.5}
                        />
                      </div>
                      <div>
                        <p className="text-[9px] text-slate-400 mb-0.5 leading-none">
                          الرصيد المتاح
                        </p>
                        <p className="font-black text-sm text-white leading-tight tracking-wide">
                ...
                <div className="relative z-10 flex items-center justify-between bg-white/5 p-2.5 rounded-xl border border-white/10 backdrop-blur-sm">
                  <p className="text-[10px] text-slate-300 max-w-[120px] leading-relaxed">
                    سجل دخولك لتجربة تسوق متكاملة
                  </p>
                  <Link
                    to="/auth"
                    onClick={onClose}

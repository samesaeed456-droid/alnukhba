...
const ProductCardInner = React.memo(function ProductCardInner({
  ...
  const isNotified = useMemo(
    () =>
      subscriptions.some(
        (s) => s.productId === p.id && s.type === "back_in_stock",
      ),
    [subscriptions, p.id],
  );
  ...
  return (
    <>
      ...
      >
        {/* Image Container */}
        <div
          className={`relative overflow-hidden mb-2 sm:mb-2.5 -mx-2 sm:-mx-2.5 -mt-2 sm:-mt-2.5 rounded-t-[20px] sm:rounded-t-[30px] bg-bg-general ${wide ? "h-[180px] sm:h-[280px]" : "aspect-square"}`}
        >
          ...
          <div className="absolute top-2 right-2 sm:top-3 sm:right-3 z-10 flex flex-col gap-1.5 sm:gap-2 pointer-events-none items-end">
            {p.isNew && (
              <div className="bg-carbon/90 backdrop-blur-md text-white px-2.5 py-1 sm:px-3.5 sm:py-1.5 rounded-full text-[8px] sm:text-[10px] font-black tracking-tight sm:tracking-normal uppercase shadow-lg border border-solar/20 flex items-center gap-1 sm:gap-1.5">
          ...
          >
            <Heart
              className={`w-3.5 h-3.5 sm:w-4 sm:h-4 drop-shadow-sm ${isWishlisted ? "fill-current" : ""}`}
            />
          </button>
        ...
        <div className="flex flex-col flex-1 px-1 sm:px-2">
          ...
          >
            <h3 className="font-bold text-carbon text-sm sm:text-base mb-1 line-clamp-2 hover:text-solar transition-colors leading-tight min-h-[40px] sm:min-h-[48px]">
              {p.name}
            </h3>

    ...
    <div className="px-2 sm:px-6 mb-6 overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-center lg:text-right flex-1 px-4 lg:px-0">
          <h2 className="text-xl lg:text-xl font-black text-carbon mb-1">
            {title}
          </h2>
          {subtitle && (
            <p className="text-titanium/60 font-medium text-sm lg:text-sm">
              {subtitle}
            </p>
          )}
        </div>
        ...
        {viewAllLink && (
          <FastLink
            to={viewAllLink}
            prefetchPage="Search"
            className="hidden lg:flex text-base font-bold text-carbon hover:underline items-center gap-1 group"
          >
            عرض الكل
      ...
      >
        ...
        >
          {products.slice(0, 8).map((p) => (
            <SwiperSlide
              key={p.id}
              className="!w-[160px] sm:!w-[220px] md:!w-[280px] !h-auto"
            >
      ...
      {viewAllLink && (
        <div className="mt-2 text-center lg:hidden">
          <FastLink
            to={viewAllLink}
            prefetchPage="Search"
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border-2 border-solar text-solar font-bold hover:bg-solar hover:text-white transition-all text-sm sm:text-base"
          >
            عرض الكل

    ...
    <div className="px-2 sm:px-6 lg:px-8 mb-4 sm:mb-6">
      ...
      <div className="flex flex-col gap-4 lg:hidden">
        <motion.div
          ...
          className="relative bg-bg-card border border-solar/30 rounded-[32px] overflow-hidden shadow-2xl group"
        >
          <FastLink to={`/product/${deals[0].id}`} className="block">
            <div className="relative h-[280px] bg-bg-general overflow-hidden">
              <FastImage
                src={deals[0].image || undefined}
                alt={deals[0].name}
              ...
              <div className="absolute top-3 right-3 flex flex-col gap-2">
                <motion.span
                  ...
                  className="bg-gold-gradient text-white px-3 py-1 rounded-full text-[10px] font-black shadow-lg flex items-center gap-1"
                >
              ...
              <div className="absolute bottom-3 left-3 right-3 p-3 bg-white/5 backdrop-blur-md flex justify-between items-center rounded-2xl border border-white/10 shadow-xl">
                <div className="flex flex-col">
                  <span className="text-[8px] text-white/90 font-black uppercase tracking-wider">
                    ينتهي العرض خلال:
                  </span>
                ...
                <div className="bg-solar text-black px-3 py-1.5 rounded-xl font-black text-xs shadow-lg animate-bounce">
                  خصم{" "}
                  {Math.round(
                    ((deals[0].originalPrice! - deals[0].price) /
            ...
            <div className="p-4">
              <h3 className="font-black text-carbon text-lg mb-2 line-clamp-1">
                {deals[0].name}
              </h3>
              <div className="mb-4">
                ...
                <div className="h-1.5 w-full bg-slate-100 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    className="h-full bg-solar rounded-full shadow-[0_0_10px_rgba(242,125,38,0.5)]"
                  />
                </div>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-baseline gap-2">
                  <span className="font-black text-2xl text-slate-900">
                    ...
                    <span className="text-sm font-bold text-solar mr-1">
                      {formatPrice(deals[0].price)
                        .split(" ")
                        .slice(1)
                        .join(" ")}
                    </span>
        ...
        <div className="relative group/scroll">
          <AnimatePresence>
            {activeDealDot === 0 && (
              <motion.div
                ...
                className="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-bg-general to-transparent pointer-events-none z-10 lg:hidden flex items-center justify-end pr-4"
              >
                <motion.div
                  animate={{ x: [0, -10, 0] }}
                  transition={{ duration: 1.5, repeat: Infinity }}
                  className="bg-solar/90 backdrop-blur-md text-black p-2 rounded-full shadow-lg border border-white/20"
                >
          ...
          <div
            ...
            className="flex overflow-x-auto pb-6 gap-3 no-scrollbar snap-x snap-mandatory px-1"
          >
            {deals.slice(1).map((product) => (
              <div
                key={product.id}
                className="min-w-[145px] w-[145px] snap-start"
              >
                <ProductCard
                  product={product}
                  className="h-full"
          ...
          <div className="absolute bottom-1 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-slate-100/50 backdrop-blur-sm px-2 py-1 rounded-full border border-slate-200/50">
            {deals.slice(1).map((_, idx) => (
              <motion.div
                key={idx}
                animate={{
                  width: activeDealDot === idx ? 12 : 4,
                  backgroundColor:
                    activeDealDot === idx ? "#E5C76B" : "#94A3B8",
                }}
      ...
      <div className="hidden lg:block mb-8">
        <div className="bg-bg-card rounded-[24px] flex flex-col lg:flex-row overflow-hidden shadow-xl border border-solar/20 hover:shadow-solar/10 hover:border-solar/40 transition-all duration-500 group">
          <div className="w-full lg:w-1/2 p-6 lg:p-8 flex flex-col justify-center relative z-10 order-2 lg:order-1">
            ...
            <h3 className="text-2xl lg:text-3xl font-black text-carbon leading-tight mb-3">
              {deals[0].name}
            </h3>
            <p className="text-slate-500 mb-6 line-clamp-2 text-sm lg:text-base leading-relaxed max-w-xl">
              {deals[0].description}
            </p>
            ...
            <div className="flex items-center justify-between mt-auto pt-5 border-t border-slate-100">
              <div className="flex items-baseline gap-3">
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-base lg:text-lg font-bold text-slate-900/70">
                    {formatPrice(deals[0].price).split(" ").slice(1).join(" ")}
                  </span>
                </div>
                {deals[0].originalPrice && (
                  <span className="text-lg lg:text-xl text-slate-400 font-medium line-through decoration-solar/30">
                    {formatPrice(deals[0].originalPrice)}
                  </span>
                )}
              </div>
              <FastLink
                to={`/product/${deals[0].id}`}
                prefetchPage="ProductDetail"
                className="inline-flex items-center justify-center gap-2 bg-gold-gradient hover:bg-gold-shimmer text-black px-6 py-3 rounded-xl text-sm lg:text-base font-black transition-all shadow-gold hover:shadow-gold-lg hover:-translate-y-0.5"
              >
          ...
          <div className="w-full lg:w-1/2 bg-bg-general flex justify-center items-center relative order-1 lg:order-2 overflow-hidden min-h-[250px] lg:min-h-[320px]">
            ...
            <motion.div
              ...
              className="relative z-10 w-full h-full flex items-center justify-center p-6 lg:p-8"
            >
              <FastImage
                src={deals[0].image || undefined}
                alt={deals[0].name}
                priority={true}
                className="w-full h-full object-contain drop-shadow-xl max-h-[280px]"
              />
              {deals[0].originalPrice && (
                <div className="absolute top-4 right-4 bg-bg-card text-white px-3 py-2 rounded-xl font-black shadow-lg border border-bg-hover flex flex-col items-center transform rotate-3">
                  ...
                  <span className="text-sm lg:text-lg text-solar leading-none mt-0.5">
                    {Math.round(
                      ((deals[0].originalPrice - deals[0].price) /
                        deals[0].originalPrice) *
                        100,

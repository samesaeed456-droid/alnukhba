...
export default React.memo(function Footer() {
  ...
  const socialLinks = [
    ...
    },
    {
      ...
      color: "hover:bg-black hover:border-black text-[#ff0050]",
    },
    {
      icon: Youtube,
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-10 lg:gap-12 mb-12">
          ...
          <div className="lg:col-span-4">
            ...
            <p className="text-sm text-slate-400 leading-relaxed mb-8 max-w-sm">
              الوجهة الأولى لتسوق أحدث الأجهزة الإلكترونية الذكية ومنظومات
              الطاقة الشمسية. نجمع بين التكنولوجيا المتطورة والجودة العالية
              لنقدم لك تجربة تسوق استثنائية في {settings.storeName}.
            </p>
              ...
              {socialLinks.length > 0 ? (
                socialLinks.map((social, idx) => (
                  <a
                    ...
                    className={`w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-slate-400 hover:text-white transition-all duration-300 ${social.color}`}
                  >
          ...
          <div className="lg:col-span-5 lg:col-start-5 grid grid-cols-2 gap-8">
              ...
              <ul className="space-y-4 text-sm">
                {[
                  ...
                  <li key={idx} className="group">
                    <FastLink
                      to={link.path}
                      prefetchPage={link.prefetch as any}
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm">
                {[
                  {
                    name: user ? "حسابي" : "تسجيل الدخول",
                    path: user ? "/profile" : "/auth",
                    prefetch: user ? "Profile" : "Auth",
                  },
                  {
                    name: "تتبع طلبك",
                    path: "/track-order",
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-4 text-sm">
              <li className="flex items-start gap-3 group">
                ...
                <span className="text-slate-400 mt-1.5">
                  {settings.address}
                </span>
              </li>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 text-sm text-slate-500">
            <p>
              &copy; {new Date().getFullYear()} {settings.storeName}. جميع
              الحقوق محفوظة.
            </p>
          ...
          <div className="flex flex-col items-center md:items-end gap-8">
            <div className="flex flex-wrap justify-center md:justify-end gap-3">
              {(
                settings.paymentMethods?.filter((m) => m.isActive && m.logo) ||
                []
              ).map((payment, idx) => (
                <motion.div
                  key={payment.id || idx}

...
export default function Header({
  ...
  const { user, notifications, cart, wishlist, products } = useStoreState();
        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between gap-4 md:gap-8">
          ...
          {!isCheckout && (
            <div className="hidden lg:flex items-center gap-8 flex-1 justify-center">
              <Link
                to="/search"
                className="flex items-center gap-2.5 text-white font-black text-[10px] uppercase tracking-widest hover:text-white transition-colors"
              >
              ...
              <nav className="flex items-center gap-8 text-[10px] font-black uppercase tracking-widest text-white/50">
                {[
                  ...
                  >
                    {item}
                    <span
                      className={`absolute bottom-0 right-0 w-0 h-0.5 bg-solar transition-all duration-300 group-hover:w-full ${idx === 1 ? "w-full" : ""}`}
                    ></span>
                  </FastLink>
                ))}
              </nav>
          ...
          {!isCheckout && (
            <div
              className="hidden md:flex items-center justify-end flex-1 max-w-xl relative"
              ref={searchRef}
            >
              <AnimatePresence mode="wait">
                {!isSearchInputFocused ? (
                      ...
                      {searchQuery.trim() !== "" && (
                        ...
                        >
                          {searchResults.length > 0 ? (
                            <div className="py-2">
                              <div className="px-5 py-2 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-bg-hover">
                                أفضل النتائج
                              </div>
                              {searchResults.map((product) => (
                                ...
                                >
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <h4 className="text-sm font-bold text-carbon truncate group-hover:text-solar transition-colors">
                                      {product.name}
                                    </h4>
                ...
                <div className="flex items-center gap-3 pr-1">
                  {user && (
                    <div className="hidden xl:flex flex-col items-end mr-1">
                      <FastLink
                        to="/profile"
                        className="text-[10px] text-white/40 font-bold uppercase tracking-wider hover:text-white transition-colors"
                      >
                        المحفظة
                      </FastLink>
                  ...
                  <FastLink
                    ...
                    prefetchPage={user ? "Profile" : "Auth"}
                    className="relative p-1 text-white/70 hover:text-white hover:bg-white/10 rounded-xl transition-all group"
                    title="حسابي"
                  >

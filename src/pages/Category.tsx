...
export default function Category() {
  ...
  const filteredProducts = useMemo(() => {
    return categoryProducts.filter((p) => {
      const matchesBrand =
        selectedBrand === "الكل" || p.brand === selectedBrand;
      const matchesSearch =
        !searchQuery.trim() ||
      ...
      <div className="max-w-[1600px] mx-auto px-2 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 sm:mb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6">
          <div className="flex-1">
            <h1 className="text-xl sm:text-2xl font-black text-carbon mb-1 sm:mb-2">
              {categoryName}
            </h1>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto hide-scrollbar pr-1">
                    {brands.map((brand) => (
                      ...
                      >
                        ...
                        <span
                          className={`text-sm ${selectedBrand === brand ? "font-bold text-carbon" : "text-titanium/80 group-hover:text-carbon"}`}
                        >
                          {brand}
                        </span>
                        <input
            ...
            {isLoading ? (
              ...
              <div className="text-center py-20 bg-white rounded-2xl border border-slate-100">
                ...
                <p className="text-slate-500">
                  عذراً، لا توجد منتجات تطابق بحثك في هذا القسم.
                </p>
                <button
                  onClick={handleResetFilters}

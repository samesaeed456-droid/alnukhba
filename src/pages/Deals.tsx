...
const DealCard = React.memo(
  ({ product, index }: { product: any; index: number; key?: React.Key }) => {
    ...
    return (
      ...
      >
        ...
        >
          <Heart
            className={`w-4 h-4 sm:w-5 sm:h-5 drop-shadow-sm ${isWishlisted ? "fill-current" : ""}`}
          />
        </button>
        ...
        <Link
          ...
          className="relative overflow-hidden bg-slate-50 block h-[180px] sm:h-[280px] w-full"
        >
          <img
            src={product.image}
            alt={product.name}
        ...
        <div className="p-2 sm:p-5 flex flex-col flex-grow">
          <Link to={`/product/${product.id}`} className="flex-grow">
            ...
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <span className="text-sm sm:text-xl font-black text-carbon">
          ...
          <div className="flex flex-col items-center justify-center text-center">
            ...
            >
              ...
              <p className="text-sm sm:text-xl text-white/70 mb-6 sm:mb-8 mx-auto">
                اكتشف أقوى العروض والخصومات على آلاف المنتجات. الكميات محدودة،
                تسوق الآن قبل نفاد المخزون!
              </p>
            </motion.div>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            >
              ...
              <p className="text-slate-500">
                عذراً، لا توجد عروض متاحة في هذا القسم في الوقت الحالي.
              </p>
            </motion.div>
          )}
        </AnimatePresence>

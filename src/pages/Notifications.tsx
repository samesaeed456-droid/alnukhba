...
export default function Notifications() {
  ...
  const handleSetNotificationToDelete = useCallback(
    (id: string) => setNotificationToDelete(id),
    [],
  );
    ...
    return (
      ...
      >
        ...
        <p className="text-slate-500 mb-8 text-center max-w-sm">
          يجب عليك تسجيل الدخول لعرض الإشعارات الخاصة بك.
        </p>
        <motion.button
      ...
      <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-2xl border-b border-slate-200/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="lg:hidden flex items-center gap-2 py-3 overflow-x-auto no-scrollbar">
            {(["all", "order", "sale", "stock", "system"] as const).map(
              (tab) => (
                <button
                  key={tab}
                  ...
                  (tab) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-8 h-8 rounded-xl flex items-center justify-center ${activeTab === tab ? "bg-white/20" : "bg-slate-100 text-slate-400"}`}
                        >
                          {tab === "all" ? (
                      ...
                      {tab === "all" && unreadCount > 0 && (
                        <span
                          className={`px-2 py-0.5 rounded-lg text-[10px] ${activeTab === tab ? "bg-white text-solar" : "bg-solar text-white"}`}
                        >
                          {unreadCount}
                        </span>
                      )}
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-black text-slate-800 uppercase tracking-wide">
                    {activeTab === "all"
                      ? "أحدث التنبيهات"
                      : `إشعارات ${activeTab === "order" ? "الطلبات" : activeTab === "sale" ? "العروض" : "المخزون"}`}
                  </h2>
                </div>
                {unreadCount > 0 && (
                  <button
                    ...
                    >
                      <div className="p-5 sm:p-6 flex gap-5">
                        ...
                        <div className="relative shrink-0">
                          <div
                            className={`w-14 h-14 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center border transition-all duration-300 group-hover:scale-105 overflow-hidden ${
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-3 mb-2">
                            <h4
                              className={`text-sm sm:text-base font-black leading-tight truncate ${notification.isRead ? "text-slate-500" : "text-slate-900"}`}
                            >
                              {notification.title}
                            </h4>
                            <div className="shrink-0 flex items-center gap-1.5 text-[10px] font-bold text-slate-400 whitespace-nowrap bg-slate-50 px-2 py-1 rounded-lg">
                              ...
                              {new Date(
                                (notification.date as any)?.seconds
                                  ? (notification.date as any).seconds * 1000
                                  : notification.date,
                              ).toLocaleTimeString("ar-u-nu-latn", {
                                hour: "2-digit",
                                minute: "2-digit",
                              })}
                            </div>
                          ...
                          <p
                            className={`text-xs sm:text-sm leading-relaxed mb-4 line-clamp-2 font-medium ${notification.isRead ? "text-slate-400" : "text-slate-600"}`}
                          >
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <button
                              onClick={() =>
                                handleSetNotificationToDelete(notification.id)
                              }
                  ...
                  {[
                    {
                      ...
                      desc: "حالة الطلب، الشحن، والتوصيل",
                      icon: <ShoppingBag className="w-4 h-4" />,
                    },
                    {
                      id: "sale",
                      label: "العروض والتخفيضات",
                      desc: "تنبيهات بخصومات حصرية وعروض لفترة محدودة",
                      icon: <Tag className="w-4 h-4" />,
                    },
                    {
                      id: "stock",
                      label: "تنبيهات المخزون",
                      desc: "إشعار عند توفر المنتجات التي تتابعها",
                      icon: <Package className="w-4 h-4" />,
                    },
                    {
                      id: "promotions",
                      label: "أخبار المتجر",
                      desc: "تحديثات عامة، ميزات جديدة، وأخبار النخبة",

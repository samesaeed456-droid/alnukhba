    ...
    >
      <div className="flex items-center justify-between mb-8">
        <motion.div variants={itemVariants} className="flex items-center gap-4">
          <Link
            to="/profile"
            className="w-10 h-10 flex items-center justify-center bg-white rounded-xl border border-slate-100 hover:bg-slate-50 transition-all shadow-sm group"
          >
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400 font-bold mt-0.5">
              سجل مشترياتك وتتبع طلباتك خطوة بخطوة
            </p>
          </div>
        </motion.div>
      </div>
      ...
      {userOrders.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm text-slate-400 font-bold mb-8 max-w-xs mx-auto">
            ابدأ التسوق الآن واكتشف أحدث المنتجات والعروض الحصرية التي نقدمها
            لك.
          </p>
          <Link
            to="/"
            className="inline-flex items-center gap-3 bg-carbon hover:bg-black text-white px-8 py-3.5 rounded-xl text-sm font-black transition-all shadow-lg shadow-carbon/20"
          >
            اذهب للتسوق
            ...
            >
              <div className="p-5 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="font-black text-carbon text-sm sm:text-base tracking-tight">
                      طلب #{order.id}
                    </div>
                    <div
                      className="text-[10px] sm:text-xs text-slate-400 font-bold mt-1 uppercase"
                      dir="ltr"
                    >
                      {new Date(
                        (order.date as any)?.seconds
                          ? (order.date as any).seconds * 1000
                          : order.date,
                ...
                <div
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] sm:text-xs font-black border uppercase shadow-sm ${getStatusColor(order.status)}`}
                >
                ...
                <div className="flex -space-x-3 space-x-reverse mb-4">
                  {order.items?.slice(0, 5).map((item, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className="w-12 h-12 rounded-xl border-2 border-white bg-white overflow-hidden relative z-[5] shadow-lg"
                      >
                        <img
                          src={itemImage}
                          alt={itemName}

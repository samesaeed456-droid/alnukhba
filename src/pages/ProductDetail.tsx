...
export default function ProductDetail() {
      ...
      if (product) {
        if (product.inStock === false) {
          ...
          setTimeout(() => {
            ...
            showToast(
              "تم تفعيل التنبيه بنجاح! سنعلمك فور توفر المنتج.",
              "success",
            );
          }, 800);
          return;
  ...
  const kitProducts = useMemo(() => {
    ...
    if (targetCategories.length > 0) {
      return products
        .filter(
          (p) => p.id !== product.id && targetCategories.includes(p.category),
        )
      ...
      <div className="flex items-center justify-between mb-6 gap-2 sm:gap-4">
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="hidden sm:flex items-center gap-2 text-[10px] sm:text-xs font-bold text-titanium/60 overflow-x-auto hide-scrollbar whitespace-nowrap">
            <FastLink to="/" className="hover:text-solar transition-colors">
              الرئيسية
            </FastLink>
            <span className="text-titanium/30">/</span>
            <span className="hover:text-solar transition-colors cursor-pointer">
              {product.category}
            </span>
            <span className="text-titanium/30">/</span>
            <span className="text-carbon truncate max-w-[150px] sm:max-w-[300px]">
              {product.name}
            </span>
          </div>
            ...
            <div className="mb-8">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div className="flex items-center gap-1 ml-auto bg-slate-50 px-2 py-1 rounded-full border border-slate-100">
                  ...
                  <span className="font-black text-carbon text-[10px]">
                    {product.rating}
                  </span>
                </div>
              </div>
                ...
                {product.inStock !== false ? (
                  ...
                  <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-bold">
                    <div className="flex items-center gap-2 text-rose-500">
                      ...
                      أكثر من {((product.id.length * 5) % 20) + 10} شخص ينتظرون
                      توفره
                    </div>
              ...
              {product.colors && product.colors.length > 0 && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-[11px] font-black text-solar bg-solar/10 px-2 py-0.5 rounded-md">
                      {selectedColor}
                    </span>
                  </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {selectedColor === color && (
                            ...
                            >
                              <Check
                                className={`w-4 h-4 ${isLight ? "text-carbon" : "text-white"}`}
                              />
                            </motion.div>
                          )}
                        </button>
              ...
              {product.sizes && product.sizes.length > 0 && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 delay-75">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    {selectedSize && (
                      <span className="text-[11px] font-black text-carbon/60">
                        {selectedSize}
                      </span>
                    )}
                  </div>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 delay-150">
                ...
                <div className="flex items-center bg-slate-50 rounded-2xl p-1.5 border border-slate-100 w-40 h-14 shadow-sm">
                  ...
                  <button
                    onClick={() =>
                      setQuantity(
                        Math.min(product.stockCount || 99, quantity + 1),
                      )
                    }
                    className="w-12 h-full flex items-center justify-center text-carbon hover:bg-white rounded-xl transition-all hover:shadow-sm active:scale-95"
                  >
              ...
              <div className="hidden sm:flex flex-col gap-6 pt-6 border-t border-slate-100">
                ...
                <div className="flex flex-col gap-3">
                  {product.inStock !== false ? (
                    ...
                    <motion.button
                      whileHover={
                        !isNotified && !isNotifying ? { scale: 1.02 } : {}
                      }
                      whileTap={
                        !isNotified && !isNotifying ? { scale: 0.98 } : {}
                      }
                      animate={
                        isNotified
                          ? {
        ...
        <div className="mt-12 sm:mt-20">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-black text-carbon tracking-tight">
                  أكمل منظومتك
                </h2>
                <p className="text-xs sm:text-sm text-titanium/40 font-bold mt-1">
                  منتجات مكملة وضرورية تعمل بشكل مثالي مع هذا المنتج
                </p>
              </div>
            </div>
        ...
        <div className="bg-white sm:rounded-[32px] border-y sm:border border-slate-100 shadow-sm overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg sm:text-xl font-black text-carbon uppercase tracking-tight">
                المواصفات التقنية
              </h3>
            </div>
            <div className="sm:hidden">
                ...
                <div className="divide-y divide-slate-50 border-t border-slate-100">
                  {Object.entries(product.specs || {}).map(
                    ([key, value], idx) => (
                      <div
                        key={idx}
                        className="grid grid-cols-2 p-4 sm:p-6 hover:bg-slate-50/50 transition-colors"
                      >
                        <span className="text-[10px] font-black text-titanium uppercase tracking-widest flex items-center">
                          {key}
                        </span>
                        <span
                          className="text-sm font-black text-carbon text-left"
                          dir="ltr"
                        >
                          {value}
                        </span>
                  ...
                  {(!product.specs ||
                    ...
                    <div className="p-8 text-center">
                      <p className="text-carbon/40 text-sm font-medium">
                        لا توجد مواصفات تقنية محددة لهذا المنتج حالياً.
                      </p>
                    </div>
                  )}
                </div>

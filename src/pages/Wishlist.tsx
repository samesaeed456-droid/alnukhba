...
export default function Wishlist() {
  ...
  const handleWishlistToggle = useCallback(
    (product: Product) => setProductToDelete(product),
    [],
  );
  ...
  if (wishlist.length === 0) {
    return (
      <motion.div className="max-w-4xl mx-auto px-4 sm:px-6 py-12 flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <h2 className="text-xl sm:text-2xl font-black text-carbon mb-3">
          قائمة المفضلة فارغة
        </h2>
        <p className="text-sm sm:text-base text-titanium/60 mb-8 text-center max-w-md leading-relaxed">
          لم تقم بإضافة أي منتجات إلى قائمة المفضلة الخاصة بك حتى الآن. تصفح
          المنتجات وإضافة ما يعجبك!
        </p>
        <Link
          to="/"

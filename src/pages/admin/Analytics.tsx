...
const Analytics = () => {
  ...
  const handleSendReminder = async (cart: any) => {
    ...
    try {
      ...
      logActivity(
        "إرسال تذكير",
        `تم إرسال تذكير للسلة المتروكة رقم ${cart.id} للعميل ${cart.customerName || "مجهول"}`,
      );
    } catch (error) {
      showToast("تم إرسال التذكير للعميل (محاكاة)", "success");
    }
  ...
  const metrics = useMemo(() => {
    ...
    const todayVisits = visits.filter(
      (v) => new Date(v.timestamp) >= todayStart,
    ).length;

    // Week's Metrics
    const weekStart = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    const weekVisits = visits.filter(
      (v) => new Date(v.timestamp) >= weekStart,
    ).length;

    // Month's Metrics
    const monthStart = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    const monthVisits = visits.filter(
      (v) => new Date(v.timestamp) >= monthStart,
    ).length;

    // Conversion Rate
    const conversionRate =
      totalVisits > 0 ? (filteredOrders.length / totalVisits) * 100 : 0;
    ...
    const singleVisitSessions = Object.values(sessionCounts).filter(
      (count) => count === 1,
    ).length;
    const bounceRate =
      Object.keys(sessionCounts).length > 0
        ? (singleVisitSessions / Object.keys(sessionCounts).length) * 100
        : 0;
    ...
    const newCustomersCount = Object.values(customerOrders).filter(
      (count) => count === 1,
    ).length;
    const returningCustomersCount = Object.values(customerOrders).filter(
      (count) => count > 1,
    ).length;
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute(
      "download",
      `alnukhba_report_${new Date().toISOString().split("T")[0]}.csv`,
    );
    ...
    <div className="space-y-10 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-black text-carbon tracking-tight">
            التحليلات والزيارات
          </h1>
          <p className="text-slate-500 font-medium">
            تحليل ذكي لحركة الزوار وأداء المتجر
          </p>
        </div>
          ...
          <div className="flex items-center justify-between mb-10">
            <div className="space-y-1">
              ...
              <p className="text-sm text-slate-400 font-medium">
                {chartView === "sales"
                  ? "مراقبة الإيرادات وحجم الطلبات"
                  : "مراقبة تدفق الزوار والزيارات الفريدة"}
              </p>
            </div>
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold text-slate-600">
                    {chartView === "sales" ? "المبيعات" : "الزيارات"}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-carbon"></div>
                  <span className="text-xs font-bold text-slate-600">
                    {chartView === "sales" ? "الطلبات" : "الفريدون"}
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-carbon p-8 rounded-[40px] text-white relative overflow-hidden shadow-xl">
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-solar">
                  مباشر الآن
                </span>
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-white/10 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold">
                    {Math.floor(metrics.avgDuration / 60)}د{" "}
                    {Math.round(metrics.avgDuration % 60)}ث
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-slate-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
          ...
          <p className="text-sm text-slate-400 font-medium mb-8">
            المنتجات التي تحقق أعلى إيرادات
          </p>
            ...
            {metrics.topProducts.map((product, idx) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-sm font-bold text-slate-600 truncate max-w-[120px] block">
                      {product.name}
                    </span>
                    <span className="text-xs text-slate-400">
        ...
        <div className="bg-white p-8 rounded-[40px] shadow-sm border border-slate-100">
          ...
          <div className="flex justify-center gap-6 mt-6">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-solar"></div>
              <span className="text-xs font-bold text-slate-600">
                جدد ({metrics.newCustomersCount})
              </span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full bg-carbon"></div>
              <span className="text-xs font-bold text-slate-600">
                عائدون ({metrics.returningCustomersCount})
              </span>
            </div>
          </div>
        </div>
            ...
            {trafficSourceData.map((source) => (
              <div key={source.name} className="space-y-2">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-black text-carbon">
                    {source.value}%
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-sm font-bold text-slate-600 block">
                      {cart.items} منتجات • {cart.customerName || "مجهول"}
                    </span>
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-black text-solar">
                      {device.value}%
                    </span>
                  </div>

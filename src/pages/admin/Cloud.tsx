    ...
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-100">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            <RefreshCw className={`w-5 h-5 ${loading ? 'animate-spin text-solar' : ''}`} />
          </button>
        </div>
      </div>
      ...
      {loading ? (
        <div className="min-h-[400px] flex flex-col items-center justify-center gap-4 text-slate-400">
          ...
          <p className="font-bold text-sm animate-pulse tracking-wide">جاري جلب ملفاتك...</p>
        </div>
      ) : (
          ...
          <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white overflow-hidden rounded-3xl border border-slate-100 shadow-sm transition-all hover:shadow-md">
                <div className="p-6 space-y-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-black text-slate-400 uppercase tracking-widest">خطة الاستخدام</span>
                    <span className="px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-wider">{usage.plan}</span>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-end justify-between">
                      <div className="space-y-1">
                        <p className="text-2xl font-black text-slate-900 leading-none">
                          ...
                          <span className="text-xs text-slate-400 font-bold mr-1">MB</span>
                        </p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">المستخدم حالياً</p>
                      </div>
                      <div className="text-right">
                        <p className="text-lg font-black text-slate-400 mb-[-2px]">
                          {Math.round(usage.storage.limit / (1024 * 1024))}
                        </p>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">الحد الأقصى</p>
                      </div>
                    </div>
                    ...
                    <div className="relative h-3 bg-slate-50 rounded-full border border-slate-100 p-0.5 overflow-hidden">
                      <div 
                        ...
                        style={{ width: `${Math.min((usage.storage.usage / usage.storage.limit) * 100, 100)}%` }}
                      ></div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-slate-50 flex items-center justify-between text-[11px] font-bold text-slate-500">
                    ...
                    <button onClick={selectUnusedImages} className="text-solar hover:underline">افحص الآن</button>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="hidden lg:block p-6 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
              <p className="text-xs leading-relaxed text-slate-400 font-medium text-center">
                يتم فحص الصور بناءً على تواجد الـ ID الخاص بها في المنتجات، الأقسام، العروض، الطلبات، والملفات الشخصية.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-sm font-black text-slate-900 flex items-center gap-2">
                جميع الصور
                <span className="text-[10px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{images.length}</span>
              </h3>
            </div>
            ...
            {images.length === 0 ? (
              <div className="bg-white border-2 border-dashed border-slate-100 rounded-[2rem] p-20 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <div className="space-y-1">
                  <h4 className="font-black text-slate-900">لا توجد صور حالياً</h4>
                  <p className="text-xs text-slate-400 max-w-xs leading-relaxed">السحابة فارغة. الصور المرفوعة عبر إضافة المنتجات ستظهر هنا تلقائياً.</p>
                </div>
              </div>
            ) : (
                ...
                {images.map((img) => (
                  ...
                  >
                    ...
                    <div className="absolute top-3 left-3 right-3 flex justify-between z-20 pointer-events-none opacity-0 translate-y-[-10px] group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                      ...
                      >
                        {selectedIds.includes(img.public_id) ? <CheckSquare className="w-4 h-4 shrink-0" /> : <Square className="w-4 h-4 shrink-0" />}
                      </button>
                    ...
                    <div className="p-4 border-t border-slate-50 space-y-1.5 bg-white">
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest truncate" title={img.public_id}>
                        {img.public_id}
                      </p>
                      <div className="flex items-center justify-between text-[9px] font-black text-slate-300 uppercase">
                        ...
                        {selectedIds.includes(img.public_id) && <span className="text-solar tracking-tighter">مختارة للتعديل</span>}
                      </div>
                    </div>
                  </div>
                ))}

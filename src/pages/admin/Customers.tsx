...
export default function Customers() {
  ...
  }>({ type: "deposit", customer: null });
  const [balanceAmount, setBalanceAmount] = useState("");
  const [balanceDescription, setBalanceDescription] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [activeTab, setActiveTab] = useState<
    "overview" | "orders" | "wallet" | "activity"
  >("overview");
  ...
  const handleUpdateBalance = (e: React.FormEvent) => {
    ...
    updateCustomerBalance(
      balanceAction.customer.uid || balanceAction.customer.phone || "",
      finalAmount,
      balanceDescription ||
        (balanceAction.type === "deposit" ? "شحن رصيد يدوي" : "سحب رصيد يدوي"),
    );
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    const cleanPhone = (newCustomer.phone || "")
      .trim()
      .replace(/\D/g, "")
      .replace(/^0+/, "");
    ...
    if (!yemenPhoneRegex.test(cleanPhone)) {
      showToast(
        "رقم الجوال غير صحيح. يجب أن يتكون من 9 أرقام تبدأ بـ 7 (مثال: 777123456)",
        "error",
      );
      return;
    }
  ...
  const stats = useMemo(() => {
    ...
    const activeThisMonth = customers.filter((c) => {
      ...
      const lastOrderDate = (lastOrder as any)?.seconds
        ? new Date((lastOrder as any).seconds * 1000)
        : lastOrder
          ? new Date(lastOrder)
          : null;
  ...
  const handleExportCSV = () => {
    ...
    const csvData = filteredCustomers.map((c) => [
      ...
      c.totalSpent || 0,
      c.joinDate
        ? `"${new Date(c.joinDate).toLocaleDateString("ar-u-nu-latn")}"`
        : "",
    ]);
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 mt-1">
              لوحة التحكم الشاملة لعملاء متجرك
            </p>
          </div>
        </div>
          ...
          >
            ...
            <div className="w-full">
              ...
              <div className="flex items-baseline justify-center sm:justify-start gap-1">
                <span className="text-xl sm:text-2xl font-black text-carbon">
                  {stats.total}
                </span>
      ...
      <div className="px-2 sm:px-8 lg:px-12 mb-8">
        ...
        <div className="flex overflow-x-auto gap-4 sm:gap-6 no-scrollbar pb-4 pt-2">
          {[
            ...
            {
              label: "جديد",
              count: customerMetrics.filter((c) => c.orderCount <= 1).length,
              icon: Star,
              color: "text-blue-500",
              bg: "bg-blue-50",
            ...
            >
              <div
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center mb-3 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3 ${
                  statusFilter === item.status
              ...
              <span
                className={`text-xs sm:text-sm font-black mb-1 ${statusFilter === item.status ? "text-white" : "text-carbon"}`}
              >
                {item.label}
              </span>
              <span
                className={`text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-lg ${
                  statusFilter === item.status
                    ? "bg-white/20 text-white"
                ...
                >
                  ...
                  <div className="flex items-center gap-4 mb-6 relative z-10">
                      ...
                      {(() => {
                        ...
                        if (img && !isPlaceholder) {
                          return (
                            <img
                              src={img}
                              alt={customer.name || customer.displayName}
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-black text-carbon truncate">
                          ...
                          return isOnline ? (
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                {lastActive
                                  ? `آخر ظهور: ${new Date(lastActive).toLocaleDateString("ar-SY", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`
                                  : "غير متصل"}
                              </span>
                            </div>
                          );
                  ...
                  <div className="space-y-3 mb-6 relative z-10">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      <span className="text-slate-600 font-medium" dir="ltr">
                        {customer.phone}
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-6 border-t border-bg-hover relative z-10 mt-auto">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-base font-black text-carbon">
      ...
      <AnimatePresence>
        {isProfileModalOpen &&
          ...
          (() => {
            const customer =
              customers.find((c) => c.phone === selectedCustomer.phone) ||
              selectedCustomer;
                            ...
                            <>
                              ...
                              >
                                <button
                                  ...
                                  className="flex items-center gap-3 w-full p-3 text-right text-sm font-semibold text-slate-600 hover:bg-slate-50 rounded-xl transition-all"
                                >
                                  ...
                                  onClick={() => {
                                    ...
                                    setConfirmModal({
                                      ...
                                      message: customer.isBlocked
                                        ? `هل أنت متأكد من إلغاء حظر العميل "${customer.name || customer.displayName}"؟`
                                        : `هل أنت متأكد من حظر العميل "${customer.name || customer.displayName}"؟`,
                                      onConfirm: () =>
                                        blockCustomer(
                                          customer.uid || customer.phone || "",
                                        ),
                                      type: customer.isBlocked
                                        ? "success"
                                        : "warning",
                                      confirmText: customer.isBlocked
                                        ? "إلغاء الحظر"
                                        : "تأكيد الحظر",
                                    });
                                  }}
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar bg-white">
                    ...
                    <div className="px-6 pb-6 pt-8 border-b border-slate-50 relative overflow-hidden">
                      ...
                      <div className="flex flex-col items-center text-center relative z-10">
                        <div className="relative mb-4">
                          ...
                          <div
                            className={`absolute -bottom-1 -right-1 w-8 h-8 rounded-full border-4 border-white flex items-center justify-center shadow-lg ${customer.isBlocked ? "bg-red-400" : "bg-emerald-400"}`}
                          >
                            {customer.isBlocked ? (
                        ...
                        <h3 className="text-xl font-bold text-carbon mb-1">
                          {customer.name || customer.displayName}
                        </h3>
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-[10px] font-bold px-3 py-1 rounded-full ${(tier.color || "").replace("text-", "text-opacity-90 text-")} flex items-center gap-1 shadow-sm`}
                          >
                            <tier.icon className="w-3 h-3" />
                            {tier.label}
                          </span>
                          <span
                            className={`text-[10px] font-bold px-3 py-1 rounded-full ${customer.isBlocked ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-500"} shadow-sm`}
                          >
                          ...
                          <span className="text-[10px] font-bold px-3 py-1 rounded-full bg-slate-50 text-slate-500 shadow-sm flex items-center gap-1">
                            ...
                            {(() => {
                              const lastVisible = customer.lastActive
                                ? new Date(customer.lastActive)
                                : null;
                              const currentlyOnline = lastVisible
                                ? lastVisible >=
                              ...
                              if (lastVisible)
                                return `آخر ظهور: ${lastVisible.toLocaleDateString("ar-SY", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`;
                      ...
                      <div className="grid grid-cols-6 grid-rows-2 gap-2 mt-8 h-44">
                        <div className="col-span-3 row-span-2 bg-slate-50/80 rounded-[2rem] p-4 flex flex-col justify-between transition-all hover:bg-slate-100 group">
                          ...
                          <div>
                            <span className="text-2xl font-bold text-carbon block leading-none mb-1">
                              {formatPrice(customer.walletBalance || 0)}
                            </span>
                        ...
                        <div className="col-span-3 row-span-1 bg-slate-50/80 rounded-[1.5rem] p-3 flex items-center gap-3 transition-all hover:bg-slate-100 group">
                          ...
                          <div>
                            <span className="text-lg font-bold text-carbon block leading-none">
                              {userOrders.length}
                            </span>
                        ...
                        <div className="col-span-3 row-span-1 bg-slate-50/80 rounded-[1.5rem] p-3 flex items-center gap-3 transition-all hover:bg-slate-100 group">
                          ...
                          <div className="min-w-0">
                            <span className="text-lg font-bold text-carbon block leading-none truncate">
                              {formatPrice(customer.totalSpent || 0)}
                            </span>
                            <span className="text-[9px] font-bold text-slate-400 uppercase">
                              إنفاق
                    ...
                    <div className="bg-white sticky top-0 z-10 px-4 border-b border-slate-50 flex items-center justify-around">
                      {[
                        ...
                        >
                          <tab.icon
                            className={`w-5 h-5 transition-transform ${activeTab === tab.id ? "scale-105" : ""}`}
                          />
                          <span
                            className={`text-[10px] font-bold ${activeTab === tab.id ? "opacity-100" : "opacity-70"}`}
                          >
                            {tab.label}
                          </span>
                          {activeTab === tab.id && (
                            <motion.div
                              layoutId="activeTab"
                              className="absolute bottom-0 left-0 right-0 h-0.5 bg-solar rounded-t-full"
                            />
                          )}
                        </button>
                      ))}
                            ...
                            <div className="space-y-3">
                              <div className="flex items-start gap-4 p-4 bg-slate-50/50 rounded-2xl border border-slate-50 group transition-all hover:bg-white hover:shadow-md hover:shadow-slate-100">
                                ...
                                <div className="flex-1">
                                  <span className="text-[10px] font-bold text-slate-400 uppercase block mb-0.5 tracking-wider">
                                    العنوان المسجل
                                  </span>
                              ...
                              <div className="flex items-start gap-4 p-4 bg-slate-50/50 rounded-2xl border border-slate-50 group transition-all hover:bg-white hover:shadow-md hover:shadow-slate-100">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-sm font-semibold text-carbon leading-relaxed">
                                    {new Date(
                                      customer.joinDate || Date.now(),
                                    ).toLocaleDateString("ar-u-nu-latn", {
                                      year: "numeric",
                                      month: "long",
                                      day: "numeric",
                                    })}
                            ...
                            <div className="space-y-2">
                              {filteredOrders.length > 0 ? (
                                filteredOrders.map((order) => (
                                  <div
                                    key={order.id}
                                    className="p-3.5 bg-white rounded-2xl border border-slate-100 flex items-center justify-between group hover:bg-slate-50 transition-all"
                                  >
                                    <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                      ...
                                      <span
                                        className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${order.status === "delivered" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                                      >
                          ...
                          >
                            <div className="p-6 sm:p-8 bg-gradient-to-br from-solar to-solar/90 rounded-[2rem] text-white shadow-lg shadow-solar/20 relative overflow-hidden">
                              <div className="relative z-10">
                                <p className="text-[10px] font-bold text-white/70 mb-1 uppercase tracking-wider">
                                  الرصيد المتاح
                                </p>
                                ...
                                <div className="flex gap-2">
                                  <button
                                    ...
                                    className="flex-1 py-3 bg-white text-solar rounded-xl font-bold text-sm shadow-sm hover:bg-slate-50 transition-all active:scale-95 flex items-center justify-center gap-2"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="flex-1 py-3 bg-black/10 backdrop-blur-sm text-white rounded-xl font-bold text-sm hover:bg-black/20 transition-all active:scale-95 flex items-center justify-center gap-2 border border-white/10"
                                  >
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between px-1">
                                ...
                                <button className="text-[10px] font-bold text-solar hover:underline">
                                  عرض الكل
                                </button>
                              </div>
                              {customer.transactions?.length ? (
                                customer.transactions
                                  ...
                                  .map((tx, idx) => (
                                    <div
                                      key={idx}
                                      className="p-3.5 bg-white rounded-2xl border border-slate-50 flex items-center justify-between shadow-sm"
                                    >
                                      <div className="flex items-center gap-3">
                                        <div
                                          className={`w-9 h-9 rounded-xl flex items-center justify-center ${tx.type === "deposit" ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600"}`}
                                        >
                                          {tx.type === "deposit" ? (
                                        ...
                                        <div>
                                          <p className="font-bold text-carbon text-xs">
                                            {tx.description ||
                                              (tx.type === "deposit"
                                                ? "إيداع رصيد"
                                                : "سحب رصيد")}
                          ...
                          >
                            ...
                            {customer.notes?.length ? (
                              [...customer.notes]
                                .sort(
                                  (a, b) =>
                                    (pinnedNotes.includes(b.id) ? 1 : 0) -
                                    (pinnedNotes.includes(a.id) ? 1 : 0),
                                )
                                .map((note) => (
                                  <div
                                    key={note.id}
                                    className={`p-4 rounded-2xl border relative overflow-hidden transition-all ${pinnedNotes.includes(note.id) ? "bg-solar/5 border-solar/20" : "bg-amber-50/30 border-amber-100/50"}`}
                                  >
                                    <div
                                      className={`absolute top-0 right-0 w-1 h-full ${pinnedNotes.includes(note.id) ? "bg-solar" : "bg-amber-400/50"}`}
                                    />
                                    <div className="flex justify-between items-start gap-2">
                                      <p
                                        className={`text-sm font-semibold leading-relaxed ${pinnedNotes.includes(note.id) ? "text-carbon" : "text-amber-900"}`}
                                      >
                                        {note.text}
                                      </p>
                                      <button
                                      ...
                                      >
                                        <Pin
                                          className={`w-3.5 h-3.5 ${pinnedNotes.includes(note.id) ? "fill-current" : ""}`}
                                        />
                                      </button>
                                    </div>
                                    <div className="flex items-center gap-2 mt-3">
                                      <Calendar
                                        className={`w-3 h-3 ${pinnedNotes.includes(note.id) ? "text-solar/70" : "text-amber-500/70"}`}
                                      />
                                      <p
                                        className={`text-[10px] font-bold ${pinnedNotes.includes(note.id) ? "text-solar/70" : "text-amber-600/70"}`}
                                      >
              ...
              <div className="p-6 sm:p-8">
                ...
                <form
                  onSubmit={(e) => {
                    e.preventDefault();
                    updateCustomer(
                      selectedCustomer.uid || selectedCustomer.phone || "",
                      editCustomerData,
                    );
                    setSelectedCustomer((prev) =>
                      prev ? { ...prev, ...editCustomerData } : null,
                    );
      ...
      <AnimatePresence>
        {isBalanceModalOpen && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div
                className={`p-6 text-white ${balanceAction.type === "deposit" ? "bg-emerald-500" : "bg-rose-500"}`}
              >
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-white/20 flex items-center justify-center text-2xl font-black">
                    {(
                      balanceAction.customer?.name ||
                      balanceAction.customer?.phone ||
                      "?"
                    ).charAt(0)}
                  </div>
            ...
            >
              <div className="p-4 sm:p-8 border-b border-gray-100 flex items-center justify-between bg-bg-general shrink-0">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div>
                    <h2 className="text-base sm:text-2xl font-black text-carbon">
                      إضافة عميل جديد
                    </h2>
                    <p className="text-[8px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mt-0.5">
                      قم بتعبئة بيانات العميل بدقة واحترافية
                    </p>
                  </div>
                </div>
                <button
              ...
              <form
                ...
                className="flex-1 overflow-y-auto p-4 sm:p-10 space-y-6 sm:space-y-10 custom-scrollbar"
              >
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10">
                  ...
                  <div className="space-y-6 sm:space-y-8">
                    <div className="flex items-center gap-3 pb-2 border-b border-gray-50">
                      ...
                      <h3 className="text-xs sm:text-sm font-black text-carbon uppercase tracking-widest">
                        البيانات الشخصية
                      </h3>
                    </div>
                    ...
                    <div className="space-y-4 sm:space-y-5">
                      <div>
                        ...
                        />
                        <p className="text-[10px] text-slate-400 mt-1 font-bold px-2">
                          يجب إدخال 4 أسماء على الأقل
                        </p>
                      </div>
            ...
            >
              ...
              <div className="mb-8 text-center">
                ...
                <p className="text-sm text-gray-500 font-bold">
                  سيصل هذا الإشعار إلى صفحة إشعارات العميل{" "}

...
export default function Dashboard() {
  ...
  const [topSellingRange, setTopSellingRange] = useState<
    "today" | "week" | "all"
  >("all");
  ...
  const inventoryHealth = useMemo(() => {
    ...
    return cats.slice(0, 3).map((cat, idx) => {
      ...
      const inStockCount = catProducts.filter(
        (p) => (p.stockCount || 0) > 0,
      ).length;
      const value =
        catProducts.length > 0
          ? Math.round((inStockCount / catProducts.length) * 100)
          : 0;
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
        ...
        >
          <div className="flex justify-between items-start w-full mb-4">
            ...
            <div
              className={`text-[10px] font-black ${Number(stats.ordersGrowth) >= 0 ? "text-emerald-600" : "text-rose-600"}`}
            >
              {Number(stats.ordersGrowth) >= 0 ? "+" : ""}
              {stats.ordersGrowth}%
            </div>
          </div>
          <div className="w-full">
        ...
        >
          <div className="flex justify-between items-start w-full mb-4">
            ...
            <div
              className={`text-[10px] font-black ${Number(stats.salesGrowth) >= 0 ? "text-emerald-600" : "text-rose-600"}`}
            >
      ...
      <motion.div
        variants={itemVariants}
        className="bg-white p-4 sm:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-6 group hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500"
      >
          ...
          {regularCustomers.slice(0, 4).map((c, i) => (
            ...
            >
              {(() => {
                ...
                return (c.name || c.displayName || c.phone || "?")
                  .charAt(0)
                  .toUpperCase();
              })()}
            </div>
      ...
      <motion.div
        variants={itemVariants}
        className="bg-white p-4 sm:p-8 rounded-[2.5rem] shadow-sm border border-slate-100"
      >
        <div className="flex flex-col sm:flex-row items-center justify-between mb-10 gap-4 text-center sm:text-right">
          <div>
            ...
            <p className="text-xs text-slate-400 font-bold mt-1">
              آخر {salesTimeRange === "weekly" ? "7 أيام" : "30 يوم"}
            </p>
          </div>
        ...
        <div
          className={`h-64 flex items-end justify-between px-1 sm:px-2 ${salesTimeRange === "monthly" ? "gap-0.5 sm:gap-2" : "gap-2 sm:gap-4"}`}
        >
            ...
            >
              <div className="relative w-full flex flex-col items-center justify-end h-48">
                <motion.div
                  initial={{ height: 0 }}
                  animate={{
                    height: `${Math.max(5, (item.value / Math.max(...salesChartData.map((v) => v.value), 1)) * 100)}%`,
                  }}
                  transition={{
                    delay: idx * (salesTimeRange === "weekly" ? 0.1 : 0.02),
                    duration: 1,
                    ease: "easeOut",
                  }}
                  className={`w-full rounded-2xl transition-all duration-500 ${
                    salesTimeRange === "monthly"
                      ? "max-w-[4px] sm:max-w-[12px]"
                      : "max-w-[32px] sm:max-w-[40px]"
                  } ${
                    idx === salesChartData.length - 1
                      ? "bg-solar shadow-lg shadow-gold"
              ...
              <span
                className={`text-[7px] sm:text-[10px] font-black uppercase tracking-widest ${idx === salesChartData.length - 1 ? "text-solar" : "text-slate-400"} ${salesTimeRange === "monthly" && idx % 5 !== 0 && idx !== salesChartData.length - 1 ? "hidden sm:block" : ""}`}
              >
                {item.label}
              </span>
            </div>
      ...
      >
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            to="/admin/products"
            className="text-solar text-xs font-black flex items-center gap-1 hover:underline"
          >
            <span>عرض الكل</span>
          ...
          {inventoryHealth.map((cat, idx) => (
            <div key={idx} className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-xs font-black ${cat.value < 20 ? "text-rose-600" : cat.value < 50 ? "text-amber-600" : "text-solar"}`}
                >
                  {cat.value}%
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <Link
              to="/admin/products"
              className="text-solar text-xs font-black hover:underline flex items-center gap-1"
            >
          ...
          <div className="space-y-4">
            {lowStockAlerts.map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 group hover:border-rose-200 transition-colors"
              >
                <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div
                    className={`px-2 py-1 rounded-lg text-[10px] font-black ${item.stockCount === 0 ? "bg-rose-100 text-rose-600" : "bg-amber-100 text-amber-600"}`}
                  >
        ...
        >
          ...
          <div className="space-y-4">
            {topSellingProducts.map((item) => (
              <div
                key={item.id}
                className="relative overflow-hidden p-4 rounded-2xl bg-slate-50 border border-slate-100 group hover:shadow-md transition-all"
              >
      ...
      >
        <div className="flex items-center justify-between mb-8">
          ...
          <Link
            to="/admin/orders"
            className="text-solar text-xs font-black hover:underline"
          >
            عرض الكل
          </Link>
        </div>
          ...
          {recentTransactions.map((item, idx) => (
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                <div
                  className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl ${item.bg} ${item.color} flex items-center justify-center group-hover:scale-110 transition-transform`}
                >
        ...
        <Link
          to="/admin/orders"
          className="block w-full mt-8 py-4 bg-slate-50 text-slate-600 text-center text-xs font-black rounded-2xl hover:bg-slate-100 transition-all border border-slate-100"
        >
          عرض جميع العمليات
        </Link>
      </motion.div>

...
import { Mail, Search, MessageCircle, Inbox, Copy, X, CheckSquare, Eye, Clock, ChevronDown } from "lucide-react";
...
const QUICK_REPLIES = [
  ...
  },
  {
    id: "details",
    label: "طلب تفاصيل",
    text: "يرجى تزويدنا بمزيد من التفاصيل حول المشكلة لنتمكن من مساعدتك بشكل أفضل.",
  },
  {
    id: "resolved",
    label: "تم الحل",
    text: "يسعدنا إبلاغك بأنه تم حل المشكلة. شكراً لتواصلك معنا.",
  },
];
  ...
  const filteredMessages = useMemo(() => {
    return supportTickets
      .filter((msg) => {
        ...
        if (statusFilter === "غير مقروءة")
          matchesFilter = msg.status === "open";
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs font-bold text-slate-400 mt-0.5 sm:mt-1">
              إدارة استفسارات وشكاوى العملاء والتواصل معهم
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-8 lg:px-12 mb-8">
        ...
        <div className="flex flex-wrap sm:flex-nowrap gap-3 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
          {[
            ...
            >
              <div
                className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center border transition-colors shrink-0 ${
                  statusFilter === stat.status
              ...
              <div>
                <p
                  className={`text-[8px] sm:text-[99px] font-black uppercase tracking-widest mb-0.5 ${statusFilter === stat.status ? "text-white/60" : "text-slate-400"}`}
                >
                  {stat.label}
                </p>
                <h3 className="text-base sm:text-lg font-black tracking-tighter leading-none">
                  {stat.value}
                </h3>
              </div>
            </motion.button>
            ...
            >
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 bg-emerald-500 text-white rounded-xl text-[10px] font-black"
                >
                  مقروءة
                </button>
                <button
                  onClick={handleBulkDelete}
                  className="px-4 py-2 bg-rose-500 text-white rounded-xl text-[10px] font-black"
                >
                  حذف
                </button>
                <button
                ...
                return (
                  <motion.div
                    ...
                    className={`bg-white border rounded-[28px] sm:rounded-[32px] p-5 sm:p-10 transition-all duration-500 flex flex-col group relative shadow-sm hover:shadow-2xl hover:-translate-y-2 overflow-hidden cursor-pointer ${
                      isUnread
                        ? "border-blue-200 hover:border-blue-300 hover:shadow-blue-500/10"
                        : "border-bg-hover hover:border-solar/30 hover:shadow-solar/10"
                    }`}
                  >
                    {/* Decorative Background Element */}
                    <div
                      className={`absolute -top-10 -right-10 w-24 sm:w-32 h-24 sm:h-32 rounded-full blur-3xl transition-colors ${
                        isUnread
                          ? "bg-blue-500/5 group-hover:bg-blue-500/10"
                          : "bg-solar/5 group-hover:bg-solar/10"
                      }`}
                    />
                    ...
                    <div className="flex items-center justify-between mb-6 sm:mb-8 relative z-10">
                      <div className="flex items-center gap-3 sm:gap-4">
                        <div
                          className={`w-10 h-10 sm:w-12 sm:h-12 rounded-[16px] sm:rounded-[20px] flex items-center justify-center transition-all duration-500 shadow-inner ${
                            isUnread
                              ? "bg-blue-50 text-blue-500 group-hover:bg-blue-500 group-hover:text-white"
                              : "bg-bg-general text-slate-400 group-hover:bg-carbon group-hover:text-solar"
                          }`}
                        >
                          <Mail className="w-5 h-5 sm:w-6 sm:h-6" />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-[9px] sm:text-[10px] font-black text-slate-300 uppercase tracking-widest mb-0.5">
                            تاريخ الاستلام
                          </span>
                    ...
                    <div className="flex items-center gap-4 sm:gap-5 mb-6 sm:mb-8 relative z-10">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-black text-carbon text-lg sm:text-xl mb-0.5 sm:mb-1 truncate leading-tight group-hover:text-solar transition-colors">
                          {msg.customerName}
                        </h3>
                    ...
                    <div className="bg-bg-general rounded-[24px] sm:rounded-[28px] p-5 sm:p-8 mb-6 sm:mb-8 flex-1 border border-bg-hover group-hover:bg-white transition-colors relative z-10">
                      <h4
                        className={`text-base sm:text-lg mb-2 sm:mb-3 truncate ${isUnread ? "font-black text-carbon" : "font-bold text-slate-700"}`}
                      >
                        {msg.subject}
                      </h4>
            ...
            >
              ...
              <div className="p-6 sm:p-12 border-b border-bg-hover bg-bg-general relative">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-8 pr-0 sm:pr-0 mt-4 sm:mt-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-xl sm:text-4xl font-black text-carbon mb-1 sm:mb-2 tracking-tight leading-tight truncate">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-4 gap-y-1 sm:gap-y-2 text-sm sm:text-base">
                      ...
                      <span
                        className="text-slate-500 font-bold block sm:inline"
                        dir="ltr"
                      >
                      ...
                      <>
                        ...
                        >
                          ...
                          {QUICK_REPLIES.map((reply) => (
                            ...
                            >
                              <div className="text-sm sm:text-base font-black text-carbon mb-1 sm:mb-1.5">
                                {reply.label}
                              </div>
                              <div className="text-xs sm:text-sm text-slate-500 font-bold line-clamp-1">
                                {reply.text}
                              </div>
                            </button>
                          ))}

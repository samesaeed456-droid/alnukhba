...
export default function Products() {
  ...
  const stats = useMemo(() => {
    ...
    const totalValue = products.reduce(
      (sum, p) => sum + p.price * (p.stockCount || 0),
      0,
    );
    ...
    return products
      .filter((product) => {
        ...
        const matchesStock =
          ...
          (stockFilter === "منخفض" &&
            product.inStock &&
            (product.stockCount || 0) <= (product.minStock || 5)) ||
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!finalFormData.sku) {
      const categoryPrefix = finalFormData.category
        ? finalFormData.category.substring(0, 3).toUpperCase()
        : "PRD";
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs font-bold text-slate-400 mt-1">
              تحكم شامل في منتجاتك، أسعارك، وحركة مخزونك بذكاء
            </p>
          </div>
        </div>
                  ...
                  {hasStockChanges && (
                    ...
                    >
                      {isSavingStock ? (
                        ...
                        <Save className="w-3 h-3 sm:w-4 sm:h-4 text-solar" />
                      )}
                ...
                <table className="w-full text-right hidden md:table">
                  <thead>
                    <tr className="bg-bg-general/50 text-slate-500 text-[10px] uppercase tracking-[0.25em] border-b border-bg-hover">
                      ...
                      <th
                        className="p-8 font-black cursor-pointer hover:text-carbon transition-colors"
                        ...
                        >
                          ...
                          <td className="p-6">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className="font-black text-carbon line-clamp-1 text-sm">
                                  {product.name}
                                </p>
                          ...
                          <td className="p-6">
                            <div className="flex flex-col">
                              {renderPrice(
                                product.price,
                                "font-black text-carbon text-base",
                              )}
                            ...
                            <div className="flex flex-col gap-2 min-w-[120px]">
                              ...
                              <div className="h-1.5 w-full bg-bg-general rounded-full overflow-hidden">
                                <motion.div
                                  initial={{ width: 0 }}
                                  animate={{
                                    width: `${Math.min(100, ((product.stockCount || 0) / 100) * 100)}%`,
                                  }}
                    ...
                    <div key={product.id} className="p-4 flex flex-col gap-4">
                      <div className="flex items-center gap-4">
                        ...
                        <img
                          src={product.image || undefined}
                          alt={product.name}
                          className="w-12 h-12 rounded-xl object-cover border border-bg-hover"
                          referrerPolicy="no-referrer"
                        />
                        <div className="flex-1 min-w-0">
                          <p className="font-black text-carbon text-sm truncate">
                            {product.name}
                          </p>
                        ...
                        <div className="flex flex-col items-end">
                          {renderPrice(
                            product.price,
                            "font-black text-carbon text-sm",
                          )}
                          <span className="text-[9px] text-slate-400 font-bold">
                            {product.category}
                          </span>
                      ...
                      <div className="flex items-center justify-between gap-4 bg-bg-general/50 p-3 rounded-xl">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            min="0"
                            value={
                              stockEdits[product.id] ?? product.stockCount ?? 0
                            }
                            onChange={(e) =>
                              handleStockChange(product.id, e.target.value)
                            }
                    ...
                    <div className="relative aspect-[4/5] overflow-hidden">
                      ...
                      {!product.inStock && (
                        <div className="absolute inset-0 bg-carbon/40 backdrop-blur-[4px] flex items-center justify-center">
                          <span className="bg-white px-3 sm:px-6 py-1 sm:py-2 rounded-full text-[8px] sm:text-[10px] font-black text-red-600 shadow-2xl uppercase tracking-widest">
                            نفذت
                          </span>
                        </div>
                      )}
                    ...
                    <div className="p-3 sm:p-6 flex-1 flex flex-col">
                      <div className="flex items-center justify-between mb-2 sm:mb-3">
                        ...
                        <div className="flex items-center gap-1 text-amber-400 bg-amber-50 px-1.5 py-0.5 rounded-lg">
                          ...
                          <span className="text-[9px] sm:text-[11px] font-black text-carbon">
                            {product.rating}
                          </span>
                        </div>
                      </div>
                      <h3 className="font-black text-carbon text-xs sm:text-lg line-clamp-1 group-hover:text-solar transition-colors duration-300">
                        {product.name}
                      </h3>
                      <div className="mt-auto pt-3 sm:pt-4 flex items-center justify-between">
                        <div className="flex flex-col">
                          {renderPrice(
                            product.price,
                            "text-sm sm:text-xl font-black text-carbon",
                          )}
                          {product.originalPrice && (
                            <span className="text-[8px] sm:text-[10px] text-slate-400 line-through font-bold">
                              {formatPrice(product.originalPrice)}
                            </span>
                          )}
                        </div>
        ...
        <div className="px-4 sm:px-8 lg:px-12">
          <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[40px] border-2 border-dashed border-bg-hover">
            ...
            <p className="text-slate-400 mt-2 font-bold text-xs uppercase tracking-widest">
              جرب تغيير معايير البحث أو إضافة منتج جديد لمتجرك
            </p>
            <button
              onClick={() => handleOpenModal()}
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-3 min-h-[32px]">
                          {formData.colors?.map((color, index) => {
                            const knownColor = PREDEFINED_COLORS.find(
                              (c) => c.name === color || c.value === color,
                            );
                            return (
                              <div
                                key={index}
                                className="flex items-center gap-1.5 bg-slate-100 border border-slate-200 px-2.5 py-1 rounded-full text-xs font-bold text-carbon"
                              >
                                {knownColor && (
                                  <span
                                    className="w-2.5 h-2.5 rounded-full border border-slate-300 shadow-sm"
                                    style={{
                                      backgroundColor: knownColor.value,
                                    }}
                                  />
                                )}
                                {!knownColor &&
                                ...
                                <span>
                                  {knownColor ? knownColor.name : color}
                                </span>
                                <button
                                  type="button"
                                  onClick={() =>
                        ...
                        <div className="flex flex-wrap gap-2 mb-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                          {PREDEFINED_COLORS.map((c) => {
                            const isSelected = formData.colors?.includes(
                              c.name,
                            );
                            return (
                              <button
                              ...
                              >
                                {isSelected && (
                                  <Check
                                    className={`w-3.5 h-3.5 ${c.value === "#FFFFFF" || c.value === "#EAB308" || c.value === "#C0C0C0" ? "text-black" : "text-white"}`}
                                  />
                                )}
                              </button>
                            );
                      ...
                      <div className="space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                        {Object.entries(formData.specs || {}).length > 0 && (
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-3">
                            {Object.entries(formData.specs || {}).map(
                              ([key, value]) => (
                                <div
                                  key={key}
                                  className="flex items-center justify-between bg-white p-2 px-3 rounded-lg border border-slate-100 shadow-sm"
                                >
            ...
            <div className="bg-carbon/90 backdrop-blur-xl border border-white/10 rounded-[28px] p-3 sm:p-4 shadow-2xl flex items-center justify-between gap-3 sm:gap-6">
              <div className="flex items-center gap-3 sm:gap-6 pr-2 sm:pr-4 border-l border-white/10">
                ...
                <div className="hidden xs:block">
                  <p className="text-white font-black text-[10px] sm:text-sm">
                    مختارة
                  </p>
                  <p className="text-slate-400 text-[8px] sm:text-[10px] font-bold uppercase tracking-widest whitespace-nowrap">
                    إجراءات جماعية
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1 sm:gap-3 flex-1 justify-center">
                <button className="p-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/10 text-white transition-all group">
                  ...
                  <span className="hidden sm:inline text-[10px] font-black uppercase tracking-widest ml-2">
                    خصم
                  </span>
                </button>
                <button className="p-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-2xl bg-white/5 hover:bg-white/10 text-white transition-all group">
